...
class MultiModalNet(nn.Module):
    def __init__(self):
        ...
        self.img_model = nn.Sequential(*self.img_model)
        self.poi_model1 = Pois_net(in_channel=1,out_channel=128,num=23,dim=args.num_class)
        self.poi_model2 = Pois_net(in_channel=1,out_channel=128,num=23,dim=args.num_class)
        #
        # if drop > 0:
        ...
        #     )
        self.img_fc_linear = nn.Linear(128, args.num_class)
        # self.cls1 = nn.Linear(5, args.num_class)  # nn.Conv1d(in_channels=256，out_channels=100,kernel_size=2)
        # self.cls2 = nn.Linear(5, args.num_class)
        # self.cls1 = nn.Conv1d(in_channels=5, out_channels=args.num_class, kernel_size=2)  # nn.Conv1d(in_channels=256，out_channels=100,kernel_size=2)
        # self.cls2 = nn.Conv1d(in_channels=5, out_channels=args.num_class, kernel_size=2)
        self.cls_cat = nn.Linear(12, args.num_class)
        self.img_fc = nn.Sequential(
            FCViewer(),
...
def main(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc_2 > best_acc_2:
            ...
            torch.save(model2, os.path.join(args.model_path, 'UFZ-resnext50_32x4d_4class.pth'))
            best_acc_2 = acc_2
            best_epoch = epoch
        # if acc_3 > best_acc_3:
...
"""
Epoch [53/100], Loss: 0.00018749, Time: 41.5sec!
Model ResNeXt - Accuracy of   住宅区 : 97 %: Correct Num: 333 in Total Num: 342
Model ResNeXt - Accuracy of 公共服务区域 : 83 %: Correct Num: 55 in Total Num: 66
Model ResNeXt - Accuracy of   商业区 : 77 %: Correct Num: 52 in Total Num: 67
Model ResNeXt - Accuracy of   工业区 : 91 %: Correct Num: 84 in Total Num: 92
Total Acc Model ResNeXt: 0.9242
----------------------------------------------------
"""
