...
class POINet(nn.Module):
    def __init__(self):
        ...
        self.cls_cat = nn.Linear(8, args.num_class)
        # self.img_fc = nn.Sequential(
        #     FCViewer(),
        #     nn.Linear(img_model.last_linear.in_features, args.num_class)
        # )

    def forward(self, x_poi_1, x_poi_2):
        ...
        # x_poi_1 = self.poi_model1_1(x_poi_1)
        # # print(x_poi_1.shape)
        # x_poi_1 = self.poi_model1_2(x_poi_1)
        # # print(x_poi_1.shape)
        # x_poi_2 = self.poi_model2_1(x_poi_2)
        # x_poi_2 = self.poi_model2_2(x_poi_2)
        ...
        # x_poi_3 = self.poi_model2(x_poi_3)
        # print(x_poi_1.shape, x_poi_2.shape)
        # x_cat = torch.cat((x_poi_1,x_poi_2),1)
        # print(x_cat.shape, x_cat.size())
        x_cat = x_cat.view(-1, x_cat.size(1))
        # x_cat = torch.cat((x_poi_3,x_cat),1)
...
def main(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for images, labels, poi_dense_h, poi_dense_w in train_loader:
            ...
            # poi_dense_area = poi_dense_area.to(device)
            ...
            # poi_dense_area = poi_dense_area.clone().detach().float()
        ...
        if acc_2 > best_acc_2:
            ...
            torch.save(model2, os.path.join(args.model_path, 'POI-UFZ-teacher-resnext50_32x4d_4class_HK.pth'))
            best_acc_2 = acc_2
            best_epoch = epoch
        # if acc_3 > best_acc_3:
